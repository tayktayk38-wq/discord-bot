...
function createHelpHome() {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `# Hello\n\n` +
      `**__I am__** **tota**, **__an entertaining and engaging Discord bot designed to bring laughter and excitement to communities.__**\n\n` +
      `**__Packed with a variety of fun features and games, ensuring that your community members never have a dull moment.__**`
    )
...
}).listen(PORT, () => console.log(`Dummy server running on port ${PORT}`));
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  if (!oldState.serverMute && newState.serverMute) {
    try {
      const fetchedLogs = await newState.guild.fetchAuditLogs({ limit: 5, type: 24 });
      ...
      if (newState.member) saveVMuteLog(newState.member.id, moderator, reason, newState.guild);
    } catch (err) {
...
client.on('messageCreate', async (message) => {
  ...
  if (prefixCommand === 'join') {
    ...
    try {
      ...
      return message.reply({ embeds: [new EmbedBuilder().setColor(EMBED_COLOR).setDescription(`**Joined ${voiceChannel.name}**`)] });
    } catch (err) {
      console.error(err);
  ...
  if (prefixCommand === 'ban') {
    ...
    try {
      if (target) {
        if (!target.bannable) return message.reply({ embeds: [new EmbedBuilder().setDescription('**I cannot ban this user.**').setColor(EMBED_COLOR)] });
